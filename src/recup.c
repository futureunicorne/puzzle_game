...
/*   Updated: 2017/07/16 19:33:17 by hel-hadi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		check_number(char *line, int on)
{
	...
	else
	{
		...
		{
			ft_putstr("le nombre de ligne doit etre superieur a 0\n");
			ft_putstr("le nombre de colone doit etre compris entre 1 et 100");
			return (-1);
		}
	}
	pos.num = ft_atoi(line);
...
int		recup_map(t_pos *pos, t_last *lst, t_last *dup)
{
	t_res res;

	ft_memset(&res, 0, sizeof(t_res));
	printf("Entre le puzzle avec un espace entre chaque chiffre (ex :\"2 3 2 3\")\n");
	while (res.i < pos->line1)
	{
		...
		if (check_line(res.dup, pos->colun) == 2)
		{
			ft_putstr("Votre puzzle est vide\n");
			return (-1);
		}
		res.cut = fill_tab(res.dup, pos->colun);
		res.rel = fill_tab(res.dup, pos->colun);
		ft_add_elm_entier(lst, res.cut, pos->colun);
		ft_add_elm_entier(dup, res.rel, pos->colun);
		res.i++;
	}
	return (0);
}
